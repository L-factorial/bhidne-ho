...
export function RoomCard({ room, member, busy, activeTables, onPress }: { room: Room; member: boolean; busy: boolean; activeTables?: number; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${member ? 'Enter' : 'Join'} ${room.name}`} disabled={busy} accessibilityState={{ disabled: busy }} onPress={onPress}
    style={{ padding: 22, minHeight: 130, borderRadius: 16, backgroundColor: c.surface, gap: 12, opacity: busy ? 0.5 : 1 }}>
    ...
    <View style={{ alignItems: 'flex-end' }}><Text style={{ color: c.accent, fontFamily: fonts.medium }}>{member ? 'Enter' : 'Join'} →</Text></View>
  </Pressable>;
}
