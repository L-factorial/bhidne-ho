...
export function HandAreaBar({ open, onToggle, count, attention, instruction }: {
  ...
  if (!open) return <Pressable accessibilityRole="button" accessibilityLabel="Expand your card area" accessibilityState={{expanded:false}} onPress={onToggle} style={barStyle}>
    {label}<View style={buttonStyle}>{chevron}</View><TurnGlow active={attention} radius={10}/>
  </Pressable>;
  return <View style={barStyle}>{label}
    <Pressable accessibilityRole="button" accessibilityLabel="Collapse your card area" accessibilityState={{expanded:true}} onPress={onToggle} style={buttonStyle}>{chevron}</Pressable>
