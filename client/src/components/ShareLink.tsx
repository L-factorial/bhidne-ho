...
export function ShareLink({ roomId, matchId, compact = false, menu = false }: { roomId: string; matchId?: string; compact?: boolean; menu?: boolean }) {
  ...
  return <View style={{ gap: 4, paddingHorizontal: compact || menu ? 0 : 8 }}>
    ...
    {(menu || !!notice) && <Text numberOfLines={menu ? 1 : undefined} accessibilityLiveRegion="polite" style={{ color: colors.text, fontSize: 12, ...(menu ? { minHeight: 16 } : {}) }}>{notice}</Text>}
  </View>;
}
