...
export function MaalReady({ hand, busy, onReview }: {
  ...
  const choices = useMemo(() => maalChoices(hand), [key]);
  ...
  const activeRoute = route === 'normal' ? (normal.length ? 'normal' : 'dublee') : (dublee.length ? 'dublee' : 'normal');
  ...
  return <>
    <View testID="marriage-maal-ready" style={{ padding: 8, gap: 4, backgroundColor: c.successSurface }}>
      <Text accessibilityLiveRegion="polite" style={{ color: c.success, fontFamily: fonts.medium }}>
        Eligible to see Maal · {choices.length} {choices.length === 1 ? 'combination' : 'combinations'} ready
      </Text>
      {button('Choose cards to show', () => setOpen(true), busy)}
    </View>
    <Modal transparent visible={open} animationType="fade" onRequestClose={() => setOpen(false)}>
      <View style={{ flex: 1, backgroundColor: c.overlay, padding: 20, justifyContent: 'center', alignItems: 'center' }}>
        <View accessibilityViewIsModal testID="marriage-maal-choices" style={{ width: '100%', maxWidth: 640, maxHeight: '90%', padding: 16, borderRadius: 16, gap: 12, backgroundColor: c.surface }}>
          <Text accessibilityRole="header" style={{ color: c.accent, fontFamily: fonts.medium, fontSize: 20 }}>Choose your route to Maal</Text>
          {button('Keep playing', () => setOpen(false))}
          <ScrollView contentContainerStyle={{ gap: 14 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {!!normal.length && button(`3 sequences / Tunnelas (${normal.length})`, () => { setRoute('normal'); setPage(0); }, false, activeRoute === 'normal')}
              {!!dublee.length && button(`7 Dublees (${dublee.length})`, () => { setRoute('dublee'); setPage(0); }, false, activeRoute === 'dublee')}
            </View>
            <Text accessibilityLiveRegion="polite" style={{ color: c.text }}>Combination {index + 1} of {options.length}</Text>
            <MarriageMeldCards groups={choice.groups} />
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {button('Previous combination', () => setPage(index - 1), index === 0)}
              {button('Next combination', () => setPage(index + 1), index === options.length - 1)}
            </View>
            {button('Review this combination', () => { setOpen(false); onReview(choice.groups); }, busy, true)}
