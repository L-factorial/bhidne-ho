...
type Ledger = { room_name?: string; players: Record<string, string>; balances: Balance[]; tables: Table[]; personal_settlements: Transfer[] };
...
export function RoomLedger({ roomId, session, embedded = false }: { roomId: string; session: Session; embedded?: boolean }) {
  ...
  const balances = (rows: Balance[]) => rows.length ? rows.map(row => <View key={row.player_id} style={styles.row}><Text style={styles.name}>{name(row.player_id)}</Text><Text style={[styles.amount, row.amount > 0 ? styles.positive : styles.negative]}>{row.amount > 0 ? '+' : ''}{row.amount}</Text></View>) : <Text style={styles.muted}>{t('ledger.noResults')}</Text>;
  ...
  const context = (tableId: string, tableName?: string, personal = false) => <View style={styles.context}>
    ...
    {personal && <Text selectable style={styles.code}>{t('ledger.roomCode', { code: roomId })}</Text>}
    <Text selectable style={styles.code}>{t('ledger.tableCode', { code: tableId })}</Text>
  </View>;
  const transfer = (row: Transfer, personal = false, table?: Table) => {
    ...
    return <View key={row.transfer_id || `${row.payer_id}:${row.payee_id}`} style={styles.transfer}>
      <View style={{ flex: 1, minWidth: 0, gap: 4 }}>
        {personal && row.table_id && context(row.table_id, source?.table_name, true)}
        <Text style={styles.name}>{name(row.payer_id)} → {name(row.payee_id)} · {row.amount}</Text>
        <Text style={styles.muted}>{row.status || 'SUGGESTED'}</Text>
      </View>
      {personal && row.status === 'OPEN' && row.payer_id === session.user_id && <Pressable disabled={busy} onPress={() => void act(row, 'mark-paid')} style={styles.action}><Text style={styles.actionText}>{t('ledger.markPaid')}</Text></Pressable>}
      {personal && row.status === 'MARKED_PAID' && row.payee_id === session.user_id && <Pressable disabled={busy} onPress={() => void act(row, 'confirm')} style={styles.action}><Text style={styles.actionText}>{t('ledger.confirmReceived')}</Text></Pressable>}
    </View>;
  };
