...
export function TableControls({ table, members, userId, busy, act, start, formationBlocked = false }: {
  table: TableView; members: string[]; userId: string; busy: boolean; formationBlocked?: boolean;
  ...
  return <View testID="table-lifecycle" style={{ backgroundColor: colors.surface, padding: 8, gap: 4 }}>
    ...
    {table.phase === 'COMPLETED' && <Text style={{ color: colors.text }}>{table.seated_players.map(p => `Seat ${p.seat_id}: ${p.display_name || p.user_id}`).join(' · ')}</Text>}
    {!!table.released_seats.length && <Text style={{ color: colors.textMuted }}>Waiting for {table.released_seats.length} replacement seat{table.released_seats.length === 1 ? '' : 's'} to be accepted.</Text>}
    <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
      {me.can_queue && !me.can_join && button('Join waitlist', () => void act('join-queue'))}
      {me.is_queued && button('Leave waitlist', () => void act('leave-queue'))}
      {table.requires_explicit_lock && table.phase === 'OPEN' && button('Lock game', () => void act('lock'), !me.can_lock || formationBlocked)}
      {table.requires_explicit_lock && table.phase === 'LOCKED' && button('Start game', () => void start(), !me.can_start || formationBlocked)}
      {me.can_leave_seat && button('Leave Seat', () => void act('leave-seat'))}
      {me.can_next_match && button('Prepare next match', () => void act('next-match'))}
      {me.can_abandon_match && button('Abandon match', () => setAbandon(true))}
      {me.can_invite_replacement && button('Invite a replacement', () => setInvite(v => !v))}
    </View>
    {formationBlocked && <Text style={{ color: colors.text }}>Save or reload your rule changes before locking or starting.</Text>}
    {offer && <View>
