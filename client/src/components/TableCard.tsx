...
export function TableCard({ table, roomId, busy, enter, compact = false, roomName }: { compact?: boolean; roomName?: string; roomId: string; table: TableSummary; busy: boolean; enter: (action: TableEntry) => void }) {
  ...
  return <View testID={`table-card-${table.match_id}`} style={{ borderRadius: radii.large, backgroundColor: c.surface, borderWidth: 1, borderColor: c.borderSubtle, marginBottom: space.md, overflow: 'hidden', opacity: busy ? 0.55 : 1 }}>
    <Pressable accessibilityRole="button" accessibilityLabel={`${primary.label} · ${table.name}`} disabled={busy} accessibilityState={{ disabled: busy }} onPress={() => enter(primary.action)} style={({ pressed }) => ({ padding: space.lg, gap: space.md, backgroundColor: pressed ? c.surfaceRaised : c.surface })}>
      ...
      <View testID={`table-preview-${table.match_id}`} style={{ height: compact ? 44 : 72, backgroundColor: compact ? 'transparent' : c.tableGreen, borderRadius: radii.xl, borderWidth: compact ? 0 : 3, borderColor: '#70533B', justifyContent: compact ? 'flex-start' : 'center', alignItems: 'center', flexDirection: 'row', gap: compact ? 4 : 10 }}>
        ...
        {compact && <View style={{ marginLeft: 'auto', minHeight: 44, paddingHorizontal: 12, backgroundColor: c.primary, borderRadius: 10, justifyContent: 'center' }}><Text style={{ color: c.onPrimary, fontFamily: fonts.medium, fontSize: 13 }}>{actionLabel} →</Text></View>}
      </View>
