...
export function MarriageHandSheet({ mobile, anchor, snap, onSnap, instruction, attention, header, children }: {
  anchor: RefObject<View | null>;
  mobile: boolean; snap: HandSnap; onSnap: (snap: HandSnap) => void;
  instruction: string; attention: boolean; header: ReactNode; children: ReactNode;
}) {
  ...
  if (!mobile) return <View ref={anchor} style={{ maxHeight: '60%' }}>{header}<ScrollView>{children}</ScrollView></View>;
  return <View ref={anchor} testID="marriage-mobile-hand" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 30,
    ...
    <View style={{ flex: 1, minHeight: 0, display: open ? 'flex' : 'none' }} accessibilityElementsHidden={!open} importantForAccessibility={open ? 'auto' : 'no-hide-descendants'}>
      {header}
      <ScrollView testID="marriage-hand-content" nestedScrollEnabled contentContainerStyle={{ paddingBottom: 16 }}>{children}</ScrollView>
    </View>
  </View>;
}
