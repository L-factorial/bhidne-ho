...
export function GameTableHeader({ title, game, onBack, endControl, children }: {
  title: string; game: string; onBack: () => void; endControl?: ReactNode; children?: ReactNode;
}) {
  ...
  return <>
    <View testID={`${game}-header`} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, padding: 8, borderBottomWidth: 1, borderColor: colors.border }}>
      ...
      <Text accessibilityRole="header" style={{ flex: 1, fontFamily: fonts.medium, fontSize: mobile ? 17 : 20, color: colors.text }}>{title}</Text>
