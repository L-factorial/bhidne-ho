  ...
  try {
    for (const kind of ['callbreak', 'marriage', 'flush']) {
      ...
      for (const width of [360, 1280]) {
        ...
        assert.equal(await overlay.getByRole('button', { name: 'Room chat', exact: true }).count(), 0);
        ...
        assert.equal(await page.getByTestId('chat-dock').count(), 0, 'room chat stays outside the game');
        await overlay.getByRole('button', { name: 'Table menu', exact: true }).click();
        await overlay.getByRole('button', { name: 'Back to room', exact: true }).click();
        await overlay.waitFor({ state: 'hidden' });
        await page.getByTestId('chat-dock').waitFor();
        await page.getByRole('button', { name: 'Room chat', exact: true }).click();
        await page.getByRole('textbox', { name: 'Room chat message', exact: true }).fill('Room-only message');
