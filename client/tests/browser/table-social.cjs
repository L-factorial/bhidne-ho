...
(async () => {
 ...
 try {
  for (const kind of ['callbreak','marriage','flush']) {
   ...
   assert.equal(await drawer.evaluate(node => getComputedStyle(node).backgroundColor), 'rgb(11, 48, 39)');
   ...
   assert.equal(await page.getByTestId('table-chat-panel').evaluate(node => getComputedStyle(node).backgroundColor), 'rgb(255, 255, 255)');
   assert.equal(await page.getByRole('heading', {name:'Table Chat', exact:true}).evaluate(node => getComputedStyle(node).color), 'rgb(33, 29, 27)');
   ...
   for (const viewport of [{width:390,height:844},{width:390,height:480},{width:1280,height:900}]) {
    ...
    assert.ok(a.x+a.width+8<=b.x+0.5 && Math.abs(a.y+a.height-b.y-b.height)<2,'Send has a separate, non-overlapping space beside the input');
    ...
    assert.equal(await input.evaluate(el=>document.activeElement===el),false,'sending blurs the input so the keyboard can close');
   ...
   assert.equal(commands.filter(c=>c.type==='TABLE_POKE_SEND').length,0,'choosing a player opens tools without sending');
   await button('Send Love').click();
   ...
   assert.equal(commands.find(c=>c.type==='TABLE_POKE_SEND').payload.reaction,'love');
   const reaction={type:'TABLE_REACTION',id:'public-reaction',room_id:'room',match_id:snapshot.match_id,sender_id:'u1',sender_name:'Hari',sender_player_id:2,recipient_id:'u0',recipient_player_id:1,reaction:'love',expires_at:Date.now()+5000};
   socket.send(JSON.stringify(reaction));
   await page.getByTestId('table-reaction-flight').waitFor();
   await page.getByTestId('table-reaction-catch').getByText('Hari sent you love!').waitFor();
   socket.send(JSON.stringify(reaction));
   assert.equal(await page.getByTestId('table-reaction-flight').count(),1,'retries must not duplicate visual effects');
