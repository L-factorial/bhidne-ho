...
class FlushCommandTarget:
    ...
    def apply(self, user_id, command):
        ...
        try:
            ...
            raise GameCommandRejected("INVALID_COMMAND", "Invalid Flush command or payload.") from error
...
def register_flush(registry, room_id, *, adapter, seat_by_user):
    """Trusted host hook: register a prepared roster, with an identical session/match ID."""
    if adapter._registered:
        raise ValueError("Each room must own a separate Flush adapter.")
    target = FlushCommandTarget(adapter, seat_by_user=seat_by_user)
